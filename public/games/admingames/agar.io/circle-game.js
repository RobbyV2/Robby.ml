...
var cg = {
  lastTime: (new Date()).getTime(),
   config: {
     ...
     autosize: true,
     circle: {
       count: 0, // per 10,000 px2
       minRadius: 0, // or 35 less than player
       maxRadius: 0, // or 15 more than player
       playerRadius: 10,
       radiusInterval: 10,
       speedScale: 3,
   ...
   init: function() {
     ...
     $(this.canvas).click(function(e) {
       if(cg.inZBounds(e.clientX,e.clientY)) {
         window.open('https://robby.ml','_blank')
       } else {
         $(cg.canvas).unbind('click')
         cg.start()
       }
   ...
   dispText: function() {
     ...
     w = this.ctx.measureText(t = 'In memorial of robby is sus').width
